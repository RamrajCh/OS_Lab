...
class FCFS():
    def __init__(self, arrival_time:list, burst_time:list):
        ...
        self.processes = pd.DataFrame({
            'process': [f'P{i+1}' for i in range(len(burst_time))],
    ...
    def algorithm(self):
        ...
        while not (all([s == "terminated" for s in list(self.processes["state"])])):
            ...
            try:
                i_job = int(temp.at[0, "index"])
                self.scheduler.loc[len(self.scheduler)] = [self.processes.at[i_job, "process"], cpu_time, cpu_time + self.processes.at[i_job, "burst_time"]]
                cpu_time += self.processes.at[i_job, "burst_time"]
